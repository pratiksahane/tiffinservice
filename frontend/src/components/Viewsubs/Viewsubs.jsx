...
const Viewsubs = ({ showViewsubs, setShowPopup8, setplanData, sellername }) => {
  ...
  const handleAvailable = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3002/api/viewsubs", userData);

      if (response.data.message === "Successfully Fetched Data") {
        alert("Data fetched successfully");
        setplanData(response.data.data); // Update meal data in App component
        setShowPopup8(false); // Close the popup after data is set
      } else {
        ...
        <div className='h-screen w-screen fixed top-0 left-0 bg-black/50 z-50 backdrop-blur-sm'>
          <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 shadow-md bg-white rounded-md duration-200 w-[300px]'>
            <div className='flex items-center justify-between'>
              <div>
                <h1 className='text-2xl font-bold text-dark'>Available Meal Subscription Plans</h1>
              </div>
              <div>
                <IoCloseOutline
                  className='text-2xl cursor-pointer'
                  onClick={() => setShowPopup8(false)}
                />
              </div>
            </div>
