...
const Availablemeal = ({ showAvailablemeal, setShowPopup4, setMealData, sellername }) => {
  ...
  const handleAvailable = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3002/api/availablemeal", userData);

      if (response.data.message === "Successfully Fetched Data") {
        alert("Data fetched successfully");
        setMealData(response.data.data); // Update meal data in App component
        setShowPopup4(false); // Close the popup after data is set
      } else {
