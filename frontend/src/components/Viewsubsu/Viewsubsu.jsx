...
const Viewsubsu = ({ showViewSubsu, setShowPopup13, setSubsData}) => {
  ...
  const handleAvailable = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3002/api/viewsubsu", userData);
      ...
      if (response.data.message === "Successfully Fetched Data") {
        ...
        setShowPopup13(false); // Close the popup after data is set
      } else {
