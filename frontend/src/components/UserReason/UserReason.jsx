...
const UserReason = ({ showUserRemove, setShowPopup18, username1}) => {
  ...
  const handleAvailable = (e) => {
    ...
    axios.post('http://localhost:3002/api/userreason', userData)
      .then(response => {
        console.log(response.data.message);
        alert("Removed User Successfully");
        setShowPopup18(false);
        setReason('');
      })
      .catch(error => {
        console.error('There was an error!', error);
        alert(`An error occurred during removing user: ${error.message}`);
      });
  };
        ...
        <div className='h-screen w-screen fixed top-0 left-0 bg-black/50 z-50 backdrop-blur-sm'>
          <div className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 shadow-md bg-white rounded-md duration-200 w-[300px]'>
            ...
            <div className='flex items-center justify-between'>
              <div>
                <h1 className='text-2xl font-bold text-dark'>Remove Service User</h1>
              </div>
              <div>
                <IoCloseOutline
                  className='text-2xl cursor-pointer'
                  onClick={() => setShowPopup18(false)}
                />
              </div>
            </div>
            {/* Signup form section */}
